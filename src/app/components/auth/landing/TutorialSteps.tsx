...
function StepText({
  ...
  return (
    <>
      ...
      <h2 className="mb-3 mt-4 text-2xl font-bold text-white">{title}</h2>
...
function MockupCard({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="mx-auto mb-8 w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-800/80 p-5 text-left shadow-2xl backdrop-blur-sm">
      <p className="mb-3 text-xs font-semibold text-gray-400">{label}</p>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="text-center">
      <motion.div
        ...
        className="mb-8 inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-600 to-purple-600 shadow-2xl"
      >
        ...
        <div className="space-y-3">
          {FIT_ROWS.map((row, i) => (
            <div key={row.symbol} className="flex items-center gap-3">
              <span className="w-12 shrink-0 text-sm font-bold text-white">{row.symbol}</span>
              <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-900/80">
                <motion.div
        ...
        <div className="flex h-28 items-end gap-2.5">
          {DIVIDEND_BARS.map((bar, i) => (
            <div key={bar.month} className="flex h-full flex-1 flex-col justify-end gap-1.5">
              ...
              />
              <span className="text-center text-[10px] text-gray-500">{bar.month}</span>
            </div>
          ))}
        </div>
      </MockupCard>
